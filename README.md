...
`.github/workflows/ci.yml` runs `swift build`, `swift test`, and `swift build -c release` on a macOS runner for every PR and every push to `main`, plus a separate `packaging-smoke` job that assembles an unsigned `Pinchos.app` and checks its bundle structure and `Info.plist` metadata — no Apple credentials involved.
